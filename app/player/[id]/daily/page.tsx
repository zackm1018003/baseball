...
// ─── Spray Chart - where balls were hit ──────────────────────────────────────
...
function SprayChart({ hitDots }: { hitDots: HitterHitDot[] }) {
  ...
  function dotColor(result: string): string {
    ...
    if (result === 'double' || result === 'triple') return '#60a5fa';
    if (result === 'home_run') return '#facc15';
    return '#f87171';
  }

  return (
    <div>
      <p className="text-[10px] font-semibold text-gray-400 uppercase text-center mb-1">Batted Ball</p>
      <svg width={W} height={H} className="rounded-lg">
        {/* Foul territory background */}
        <rect width={W} height={H} fill="#1a2035" rx="6" />

        {/* Fair territory grass */}
        <path d="M 125 195 L 8 162 A 117 117 0 0 0 242 162 Z" fill="#1e4620" />

        {/* Infield dirt */}
        <ellipse cx="125" cy="163" rx="48" ry="38" fill="#6b4f2a" opacity="0.65" />

        {/* Foul lines */}
        <line x1="125" y1="195" x2="8" y2="162" stroke="white" strokeWidth="1" opacity="0.45" />
        <line x1="125" y1="195" x2="242" y2="162" stroke="white" strokeWidth="1" opacity="0.45" />

        {/* Outfield wall */}
        <path d="M 8 162 A 117 117 0 0 0 242 162" fill="none" stroke="white" strokeWidth="1.5" opacity="0.65" />

        {/* Base paths (diamond) */}
        <polygon points="125,195 162,162 125,129 88,162" fill="none" stroke="white" strokeWidth="0.8" opacity="0.45" />

        {/* Pitcher's mound */}
        <circle cx="125" cy="163" r="5" fill="#8b6914" opacity="0.75" />

        {/* Bases */}
        <polygon points="125,191 129,195 125,199 121,195" fill="white" opacity="0.85" />
        <rect x="159.5" y="159.5" width="5" height="5" fill="white" opacity="0.85" />
        <rect x="122.5" y="126.5" width="5" height="5" fill="white" opacity="0.85" />
        <rect x="85.5" y="159.5" width="5" height="5" fill="white" opacity="0.85" />
        ...
        {/* No data message */}
        {hitDots.length === 0 && (
          <text x={W / 2} y={H / 2 + 4} textAnchor="middle" fontSize="10" fill="#6b7280">
            No Statcast data
          </text>
        )}

        {/* Legend */}
        <circle cx="12" cy="207" r="3" fill="#22c55e" />
        <text x="18" y="211" fontSize="7.5" fill="#9ca3af">1B</text>
        <circle cx="37" cy="207" r="3" fill="#60a5fa" />
        <text x="43" y="211" fontSize="7.5" fill="#9ca3af">2B/3B</text>
        <circle cx="75" cy="207" r="3" fill="#facc15" />
        <text x="81" y="211" fontSize="7.5" fill="#9ca3af">HR</text>
        <circle cx="100" cy="207" r="3" fill="#f87171" />
        <text x="106" y="211" fontSize="7.5" fill="#9ca3af">Out</text>
      </svg>
    </div>
  );
}
