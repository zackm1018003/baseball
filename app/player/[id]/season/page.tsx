...
function MiniPercentileBar({ value, leagueKey, level, pa, minPa = 25, baselineOverride, light = false, colorMode = 'redblue' }: {
  ...
  light?: boolean;
  // 'redblue' (default) = the usual good/bad semantic (red=above avg, blue=below avg).
  // 'mono' = same magnitude bars but black/gray both sides — for scales where one side
  // isn't inherently "better" (e.g. a platoon-direction gauge).
  colorMode?: 'redblue' | 'mono';
}) {
  const LG       = getLG(level);
  ...
  const blueColors = colorMode === 'mono'
    ? (light ? ['#999999','#777777','#555555','#333333','#111111'] : ['#8a8a8a','#6e6e6e','#525252','#363636','#1a1a1a'])
    : ['#1d7ab4','#1a6196','#184f82','#174678','#163d6e'];
  const redColors  = colorMode === 'mono'
    ? (light ? ['#999999','#777777','#555555','#333333','#111111'] : ['#8a8a8a','#6e6e6e','#525252','#363636','#1a1a1a'])
    : ['#9e0808','#c41515','#e82525','#f72e2e','#ff2d2d'];
...
function ZoneHeatChart({ zoneStats, light }: { zoneStats: ZoneStat[]; light?: boolean }) {
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: 272, height: 272 }}>
      <svg width={272} height={272} viewBox={`0 0 ${size} ${size}`} style={{ background: light ? '#ffffff' : '#0f1117', borderRadius: 2 }}>
...
export default function HitterSeasonPage({ params }: SeasonPageProps) {
  ...
  const [mlbLaHardBase,  setMlbLaHardBase]  = useState<{ mean: number; std: number }>({ mean: 13.5,  std: 8.0 });
                ...
                return (
                  <div key={key}>
                    <div className={`text-center px-2 py-1 border-b ${th.border} ${i > 0 ? `border-t ${th.border}` : ''}`} style={{ background: th.statsBg }}>
                      <span className="text-[11px] font-bold uppercase tracking-widest" style={{ color: light ? '#111111' : '#ff2d2d' }}>
                        {s ? `${s.pa} PAs ` : ''}{label}
                      </span>
                    </div>
              ...
              {data.splits.vsLHP && data.splits.vsRHP && (() => {
                ...
                return (
                  <div className={`border-t ${th.border} px-3 py-2`} style={{ background: th.statsBg }}>
                    ...
                    <MiniPercentileBar value={combinedZ} leagueKey="_" baselineOverride={{ mean: 0, std: 1 }} level={data?.level} pa={totalPa} minPa={40} light={light} colorMode="mono" />
                  </div>
                );
              })()}
            </div>
