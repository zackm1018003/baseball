...
function GradeBadge({ val }: { val: string | undefined }) {
  ...
  return (
    <span
      className="inline-block px-2 py-0.5 text-xs font-bold text-ink min-w-[2.5rem] text-center"
...
function GradeSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <select
      ...
      className="rounded py-1 px-1 text-sm font-bold text-center border border-ink/20 focus:outline-none focus:border-amber-500 cursor-pointer w-16"
      style={{
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              editMode ? 'bg-deep text-deep-fg' : 'bg-panel text-ink-3 hover:bg-bone hover:text-ink'
            }`}
          >
        ...
        {entries.length === 0 ? (
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              {/* Type filter */}
              <div className="flex rounded overflow-hidden border border-ink/20">
              ...
              <table className="w-full text-sm whitespace-nowrap">
                <thead>
                  <tr className="border-b border-ink/20">
                    ...
                    <th className="text-left px-3 py-2.5 text-ink-3 font-medium cursor-pointer hover:text-ink" onClick={() => handleColClick('name')}>
                    ...
                    <th className="text-center px-3 py-2.5 text-ink-3 font-medium cursor-pointer hover:text-ink" onClick={() => handleColClick('draftYear')}>
                        ...
                        {editMode ? (
                          <div className="flex flex-col gap-0.5 items-center">
                            <button
                              ...
                              disabled={i === 0}
                              className="text-ink-4 hover:text-ink disabled:opacity-20 disabled:cursor-not-allowed leading-none text-[10px] px-1"
                              title="Move up"
                            >▲</button>
